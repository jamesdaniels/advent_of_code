const input = `faded yellow bags contain 4 mirrored fuchsia bags, 4 dotted indigo bags, 3 faded orange bags, 5 plaid crimson bags.
...
const parse = (input:string) => input.split('.').map(it => {
    // no matchAll
...
let combined: string[] = parsed.filter(it => !!it.contains.find(it => it.color === 'shiny gold')).map(it => it.color);

while(true) {

    const nextPass = combined.map(color => parsed.filter(it => !!it.contains.find(it => it.color === color)).map(it => it.color));
...
const countBags = (color: string) => parsed.find(it => it.color === color).contains.reduce((acc, bag) =>
